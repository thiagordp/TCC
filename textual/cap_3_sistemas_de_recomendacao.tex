...
Os SRs são subdivididos em abordagens, entre elas, a filtragem colaborativa, a baseada em conteúdo, a baseada em conhecimento entre outras. Estas diferem na forma como geram sugestões e nas informações que utilizam para a realização de sugestões \cite{Jannach2010}. 
...
\section{Filtragem Colaborativa (FC)} \label{sec:filtragem-colaborativa}
    ...
    \subsection{Método de FC Baseado em Memória}
        ...
        \subsubsection{Vizinhos Mais Próximos}
            ...
            A partir de cálculos de similaridade o algoritmo encontrará os $k$ pontos mais próximos, onde tal valor varia de acordo o contexto da aplicação. \citeonline{Duda2000} sugerem um $k$  igual a raiz quadrada do total de pontos $n$, ou seja, $k$ é igual $\sqrt{n}$, no âmbito geral de classificadores, já no contexto de SRs, onde há bases com milhares de usuários, \citeonline{Jannach2010} afirma que $k$ entre $20$ a $50$ é uma boa estimativa. 
...
\section{Abordagem Baseada em Conteúdo}
   ...
   % Um padrão seguido para SR baseada em conteúdo para documentos textuais é usar o conteúdo em si do item para gerar recomendação e não os metadados, ou seja, as informações sobre o item. 
    
    %Como forma de encontrar os termos mais relevantes em um documento, pode-se utilizar o cálculo do TF-IDF. A partir deste é construído um vetor de pesos ao qual representa o item, onde cada posição corresponde a um termo com sua respectiva relevância \cite{Manning2008}.
    
    
    % VSM = vetor
    % TF-DEF = forma de cálculo
...
\section{Abordagem H\'ibrida}
    ...
    O processo de hibridização de um SR, ou seja, a mesclagem de diversas abordagens, pode seguir algum método específico que indique como e quando os itens de recomendação de cada abordagem serão utilizados. \citeonline{Jannach2010} expõe três métodos para tornar SRs híbridos: monolítico, paralelo e sequencial, cada um com suas respectivas subdivisões.
